...
def insert_headlines(headlines: list[dict[str, Any]]) -> int:
    """Insert raw headlines (deduped by URL). Returns inserted count."""
    if not headlines:
        return 0
    inserted = 0
    with get_connection() as conn:
        for h in headlines:
            try:
                conn.execute(
                    """INSERT INTO news_headlines (url, title, source, published_at)
                       VALUES (?, ?, ?, ?)
                       ON CONFLICT (url) DO NOTHING""",
                    (h["url"], h["title"], h.get("source"), h["published_at"]),
                )
                inserted += 1
            except Exception:
                continue
...
def pending_headlines(limit: int = 50) -> list[dict[str, Any]]:
    with get_connection() as conn:
        rows = conn.execute(
...
def mark_headlines_classified(ids: list[int]) -> None:
    if not ids:
        return
    placeholders = ",".join(["%s"] * len(ids))
    with get_connection() as conn:
        conn.execute(
            f"UPDATE news_headlines SET classified = true WHERE id IN ({placeholders})",
            tuple(ids),
        )


def insert_theme_news(items: list[dict[str, Any]]) -> int:
    """Insert classified theme-news links."""
    if not items:
        return 0
    inserted = 0
    with get_connection() as conn:
            ...
            try:
                conn.execute(
                    """INSERT INTO theme_news (
                           theme_id, published_at, title, url, source,
                           matched_keywords, classified_by)
                       SELECT id, ?, ?, ?, ?, ?, ?
                         FROM themes WHERE code = ?""",
                    (
                        item["published_at"],
                        item["title"],
                        item.get("url"),
...
def news_count_24h_by_theme() -> dict[str, int]:
    """Returns {theme_id: count of news in past 24h}."""
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT theme_id, COUNT(*) AS c
                 FROM theme_news
                WHERE published_at > now() - interval '24 hours'
                GROUP BY theme_id"""
        ).fetchall()
    return {str(r["theme_id"]): r["c"] for r in rows}


def recent_news_for_theme(theme_id: str, limit: int = 5) -> list[dict[str, Any]]:
    with get_connection() as conn:
        rows = conn.execute(
